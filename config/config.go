...
type Config struct {
	...
	SupplyFilterEnabled  bool       `json:"supply_filter"` //過濾補八字
	ZodiacFilterEnabled  bool       `json:"zodiac_filter"` //過濾生肖
	TrigramFilterEnabled bool       `json:"bagua_filter"`  //過濾卦象
...
var DefaultJSONPath = ""
var DefaultHeads = []string{"姓名", "筆畫", "拼音", "喜用神", "八字"}
...
func LoadConfig() (c *Config) {
	...
	c = &Config{}
	if e := json.Unmarshal(bys, &c); e != nil {
		return def
	}
	return c
}
...
func DefaultConfig() *Config {
	return &Config{
		...
		HardFilter:           false,
		FixBazi:              false,
		SupplyFilterEnabled:  true,
		ZodiacFilterEnabled:  true,
		TrigramFilterEnabled: true,
		Regular:              true,
		Database: Database{
			Host:         "127.0.0.1",
			Port:         "3306",
